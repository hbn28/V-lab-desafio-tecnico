...
4. Com `APP_SEED=false`, crie uma conta administrativa no serviço backend. No Railway, abra o terminal SSH do serviço (ou use `railway ssh` com o serviço backend selecionado) e execute `php artisan operadores:criar`. Informe nome, e-mail e senha apenas nos prompts interativos. A senha deve ter pelo menos 12 caracteres, com maiúscula, minúscula, número e símbolo; ela não aparece no terminal. O comando recusa e-mail já cadastrado e não altera contas existentes. Não coloque credenciais em Variables, comandos, GitHub ou capturas de tela.
